...
@component('components.card')
    ...
    @slot('subtitle')
      As of {{ $data->current->last_updated->format('g:i a T') }}
    @endslot
    ...
    @foreach ($data->forecast->forecast_days as $forecastDay)
      ...
      <?php $forecast = collect($forecastDay->hours)
        ->filter(function ($hour) {
          return $hour->time->diffInHours(now()->startOfHour(), false) <= 3;
        })
        ->map(function ($hour) {
          return [
            ...
            'expandable' => $hour->time->diffInHours(now()->startOfHour(), false) <= 0,
            'expanded' => $hour->time->diffInHours(now()->startOfHour()) == 0
          ];
        }); ?>
